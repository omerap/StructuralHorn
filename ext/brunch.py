...
def main (argv):
    ...
    #if not os.path.exists (os.path.join (args.out, 'out')):
    #	os.mkdir (os.path.join (args.out, 'out'))
    	
    #if not os.path.exists (os.path.join (args.out, 'stats')):
    #	os.mkdir (os.path.join (args.out, 'stats'))
    ...
    for f in args.file:
        if ".list" in f:
            ...
            for l in F:
                l = l.strip()
                runTool (args.tool_args, l, args.out,
                         cpu=args.cpu,
                         mem=args.mem,
                         fmt=fmt)
        else:
